...
import org.apache.spark.{Logging, SparkConf, SparkContext, Accumulator}
...
def tsqr(sc: SparkContext,matrix:RDD[org.apache.spark.mllib.linalg.Vector], numRows: Int, numCols: Int, numParts:Int, numClasses: Int){
  //def tsqr(args: Array[String]) {
  ...
  /*
    ...
    */
  val rowsPerPart = numRows/numParts
  val matrixParts = matrix.repartition(numParts).mapPartitions{part =>
    val mat = new org.apache.spark.mllib.linalg.DenseMatrix[Double](rowsPerPart, numCols, part)
    Iterator(mat)
  }
